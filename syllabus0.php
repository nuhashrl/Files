...
if(!empty($_SESSION['usertype'])){
    if(!empty($_FILES['syllabus'])){
       ...
       // echo $sqlPortfolio;
        ...
        $fileTmpName = $_FILES['syllabus']['tmp_name'];
        $fileSize = $_FILES['syllabus']['size'];
        $fileError = $_FILES['syllabus']['error'];
        $fileType = $_FILES['syllabus']['type'];
        ...
        if($fileError === 0){
            ...
            if($fileSize < 5000000){
               //add sql to insert in db
                ...
                $fileDestination = 'uploads/syllabus/'.$fileNameNew;
                move_uploaded_file($fileTmpName, $fileDestination);
                $sqlUpdateTimetable = "UPDATE portfolio SET syllabus = '$fileDestination' WHERE idportfolio = '$idportfolio'";
                //echo $sqlUpdateKPI;
                ...
                header("Location: test-img.php?uploadsuccess");
            }else{
