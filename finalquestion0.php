...
if(!empty($_SESSION['usertype'])){
    if(!empty($_FILES['finalquestion'])){
       ...
       // echo $sqlPortfolio;
        ...
        $fileName = $_FILES['finalquestion']['name'];
        $fileTmpName = $_FILES['finalquestion']['tmp_name'];
        $fileSize = $_FILES['finalquestion']['size'];
        $fileError = $_FILES['finalquestion']['error'];
        $fileType = $_FILES['finalquestion']['type'];
        ...
        if($fileError === 0){
            ...
            if($fileSize < 5000000){
               //add sql to insert in db
                ...
                $fileDestination = 'uploads/finalquestion/'.$fileNameNew;
                move_uploaded_file($fileTmpName, $fileDestination);
                $sqlUpdateTimetable = "UPDATE portfolio SET finalQuestion = '$fileDestination' WHERE idportfolio = '$idportfolio'";
                //echo $sqlUpdateKPI;
                ...
                header("Location: finalquestion.php?uploadsuccess");
            }else{
