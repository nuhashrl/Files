...
if(!empty($_SESSION['usertype'])){
    if(!empty($_FILES['answerscheme'])){
       ...
       // echo $sqlPortfolio;
        ...
        $fileName = $_FILES['answerscheme']['name'];
        $fileTmpName = $_FILES['answerscheme']['tmp_name'];
        $fileSize = $_FILES['answerscheme']['size'];
        $fileError = $_FILES['answerscheme']['error'];
        $fileType = $_FILES['answerscheme']['type'];
        ...
        if($fileError === 0){
            ...
            if($fileSize < 5000000){
               //add sql to insert in db
                ...
                $fileDestination = 'uploads/answerscheme/'.$fileNameNew;
                move_uploaded_file($fileTmpName, $fileDestination);
                $sqlUpdateTimetable = "UPDATE portfolio SET answerscheme = '$fileDestination' WHERE idportfolio = '$idportfolio'";
                //echo $sqlUpdateKPI;
                ...
                header("Location: answerscheme.php?uploadsuccess");
            }else{
