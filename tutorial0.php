...
if(!empty($_SESSION['usertype'])){
    if(!empty($_FILES['tutorial'])){
       ...
       // echo $sqlPortfolio;
        ...
        $fileName = $_FILES['tutorial']['name'];
        $fileTmpName = $_FILES['tutorial']['tmp_name'];
        $fileSize = $_FILES['tutorial']['size'];
        $fileError = $_FILES['tutorial']['error'];
        $fileType = $_FILES['tutorial']['type'];
        ...
        if($fileError === 0){
            ...
            if($fileSize < 5000000){
               //add sql to insert in db
                ...
                $fileDestination = 'uploads/tutorial/'.$fileNameNew;
                move_uploaded_file($fileTmpName, $fileDestination);
                $sqlUpdateTimetable = "UPDATE portfolio SET tutorial = '$fileDestination' WHERE idportfolio = '$idportfolio'";
                //echo $sqlUpdateKPI;
                ...
                header("Location: tutorial.php?uploadsuccess");
            }else{
