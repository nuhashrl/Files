...
if(!empty($_SESSION['usertype'])){
    if(!empty($_FILES['test'])){
       ...
       // echo $sqlPortfolio;
        ...
        $fileTmpName = $_FILES['test']['tmp_name'];
        $fileSize = $_FILES['test']['size'];
        $fileError = $_FILES['test']['error'];
        $fileType = $_FILES['test']['type'];
        ...
        if($fileError === 0){
            ...
            if($fileSize < 5000000){
               //add sql to insert in db
                ...
                $fileDestination = 'uploads/test/'.$fileNameNew;
                move_uploaded_file($fileTmpName, $fileDestination);
                $sqlUpdateTimetable = "UPDATE portfolio SET test = '$fileDestination' WHERE idportfolio = '$idportfolio'";
                //echo $sqlUpdateKPI;
