...
error_reporting(E_ALL);
    if(isset($_POST['submit'])){
        ...
        $errors = array();
        $philo = mysqli_real_escape_string($dbconn, $_POST['philosophy']);
        /**
        ...
        if(isset($smtSes)){
            ...
            if(mysqli_num_rows($qryPort) > 0){
                ...
                $sqlUpdate = "UPDATE portfolio SET philosophy = '$philo' WHERE idportfolio = '$idPort'";
                ...
                $sqlInsert = "INSERT INTO portfolio(edit_DATETIME, SEMESTER_ID, systemuser_idsystemuser, philosophy) 
                VALUES('$currentdate', '$smtSes', '$iduser', '$philo')"; 
        ...
        if(sizeof($errors) > 0){
            ...
            echo "<script>window.alert('ERRORS: ".$errors."');window.location.href='./edit-philosophy.php';</script>";
        }else{
            echo "<script>window.alert('SUCCESFULLY UPDATED');window.location.href='./edit-philosophy.php'</script>";
        }
        //echo $philo;
    }//end if isset(submit)
    else{
        header("location: ./edit-philosophy.php");
    }
?>