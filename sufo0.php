...
if(!empty($_SESSION['usertype'])){
    if(!empty($_FILES['sufo'])){
       ...
       // echo $sqlPortfolio;
        ...
        $fileName = $_FILES['sufo']['name'];
        $fileTmpName = $_FILES['sufo']['tmp_name'];
        $fileSize = $_FILES['sufo']['size'];
        $fileError = $_FILES['sufo']['error'];
        $fileType = $_FILES['sufo']['type'];
        ...
        if($fileError === 0){
            ...
            if($fileSize < 5000000){
               //add sql to insert in db
                ...
                $fileDestination = 'uploads/sufo/'.$fileNameNew;
                move_uploaded_file($fileTmpName, $fileDestination);
                $sqlUpdateTimetable = "UPDATE portfolio SET sufo = '$fileDestination' WHERE idportfolio = '$idportfolio'";
                //echo $sqlUpdateKPI;
                ...
                header("Location: sufo.php?uploadsuccess");
            }else{
