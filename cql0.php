...
if(!empty($_SESSION['usertype'])){
    if(!empty($_FILES['cql'])){
       ...
       // echo $sqlPortfolio;
        ...
        $fileName = $_FILES['cql']['name'];
        $fileTmpName = $_FILES['cql']['tmp_name'];
        $fileSize = $_FILES['cql']['size'];
        $fileError = $_FILES['cql']['error'];
        $fileType = $_FILES['cql']['type'];
        ...
        if($fileError === 0){
            ...
            if($fileSize < 5000000){
               //add sql to insert in db
                ...
                $fileDestination = 'uploads/cql/'.$fileNameNew;
                move_uploaded_file($fileTmpName, $fileDestination);
                $sqlUpdateTimetable = "UPDATE portfolio SET cql = '$fileDestination' WHERE idportfolio = '$idportfolio'";
                //echo $sqlUpdateKPI;
                ...
                header("Location: cql.php?uploadsuccess");
            }else{
