...
if(!empty($_SESSION['usertype'])){
    if(!empty($_FILES['attendance'])){
       ...
       // echo $sqlPortfolio;
        ...
        $fileTmpName = $_FILES['attendance']['tmp_name'];
        $fileSize = $_FILES['attendance']['size'];
        $fileError = $_FILES['attendance']['error'];
        ...
        if($fileError === 0){
            ...
            if($fileSize < 5000000){
               //add sql to insert in db
                ...
                $fileDestination = 'uploads/attendance/'.$fileNameNew;
                move_uploaded_file($fileTmpName, $fileDestination);
                $sqlUpdateTimetable = "UPDATE portfolio SET studAttend = '$fileDestination' WHERE idportfolio = '$idportfolio'";
                //echo $sqlUpdateKPI;
                ...
                header("Location: attendance.php?uploadsuccess");
            }else{
