...
if(!empty($_SESSION['usertype'])){
    if(!empty($_FILES['timetable'])){
       ...
       // echo $sqlPortfolio;
        ...
        $fileSize = $_FILES['timetable']['size'];
        $fileError = $_FILES['timetable']['error'];
        $fileType = $_FILES['timetable']['type'];
        ...
        if($fileError === 0){
            ...
            if($fileSize < 5000000){
               //add sql to insert in db
                ...
                $fileDestination = 'uploads/timetable/'.$fileNameNew;
                move_uploaded_file($fileTmpName, $fileDestination);
                $sqlUpdateTimetable = "UPDATE portfolio SET timetable = '$fileDestination' WHERE idportfolio = '$idportfolio'";
                //echo $sqlUpdateKPI;
                ...
                header("Location: academic-timetable.php?uploadsuccess");
            }else{
