...
if(!empty($_SESSION['usertype'])){
    if(!empty($_FILES['quiz'])){
       ...
       // echo $sqlPortfolio;
        ...
        $fileTmpName = $_FILES['quiz']['tmp_name'];
        $fileSize = $_FILES['quiz']['size'];
        $fileError = $_FILES['quiz']['error'];
        $fileType = $_FILES['quiz']['type'];
        ...
        if($fileError === 0){
            ...
            if($fileSize < 5000000){
               //add sql to insert in db
                ...
                $fileDestination = 'uploads/quiz/'.$fileNameNew;
                move_uploaded_file($fileTmpName, $fileDestination);
                $sqlUpdateTimetable = "UPDATE portfolio SET quiz = '$fileDestination' WHERE idportfolio = '$idportfolio'";
                //echo $sqlUpdateKPI;
                ...
                header("Location: quiz.php?uploadsuccess");
            }else{
