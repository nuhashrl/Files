...
if(!empty($_SESSION['usertype'])){
    if(!empty($_FILES['finalresult'])){
       ...
       // echo $sqlPortfolio;
        ...
        $fileName = $_FILES['finalresult']['name'];
        $fileTmpName = $_FILES['finalresult']['tmp_name'];
        $fileSize = $_FILES['finalresult']['size'];
        $fileError = $_FILES['finalresult']['error'];
        $fileType = $_FILES['finalresult']['type'];
        ...
        if($fileError === 0){
            ...
            if($fileSize < 5000000){
               //add sql to insert in db
                ...
                $fileDestination = 'uploads/finalresult/'.$fileNameNew;
                move_uploaded_file($fileTmpName, $fileDestination);
                $sqlUpdateTimetable = "UPDATE portfolio SET finalresult = '$fileDestination' WHERE idportfolio = '$idportfolio'";
                //echo $sqlUpdateKPI;
                ...
                header("Location: finalresult.php?uploadsuccess");
            }else{
