...
if(!empty($_SESSION['usertype'])){
    if(!empty($_FILES['labtutorial'])){
       ...
       // echo $sqlPortfolio;
        ...
        $fileName = $_FILES['labtutorial']['name'];
        $fileTmpName = $_FILES['labtutorial']['tmp_name'];
        $fileSize = $_FILES['labtutorial']['size'];
        $fileError = $_FILES['labtutorial']['error'];
        $fileType = $_FILES['labtutorial']['type'];
        ...
        if($fileError === 0){
            ...
            if($fileSize < 5000000){
               //add sql to insert in db
                ...
                $fileDestination = 'uploads/labtutorial/'.$fileNameNew;
                move_uploaded_file($fileTmpName, $fileDestination);
                $sqlUpdateTimetable = "UPDATE portfolio SET labtutorial = '$fileDestination' WHERE idportfolio = '$idportfolio'";
                //echo $sqlUpdateKPI;
                ...
                header("Location: labtutorial.php?uploadsuccess");
            }else{
